    ...
    for (i=0; i < gems.length; i++){
      ...
      if (gem_code[0] == "gem" && !gem_code.includes("path:") && !gem_code.includes("git:")){
        ...
        promises.push(detective.cycle_gem_versions(gem_title, i).then(function(version_data){
          ...
          current_version_number = gem_version_list.indexOf(current_version);
          range = editor.getBuffer().rangeForRow(version_data["row"]);
          if(current_version_number == 0){
            // do nothing
          } else if(current_version_number == 1){
            marker = editor.markBufferRange(range, {invalidate: 'inside'});
            decoration = editor.decorateMarker(marker, {type: 'line', class: "gem_checker_info"});
            ...
            marker = editor.markBufferRange(range, {invalidate: 'inside'});
            decoration = editor.decorateMarker(marker, {type: 'line', class: "gem_checker_error"});
            ...
            marker = editor.markBufferRange(range, {invalidate: 'inside'});
            decoration = editor.decorateMarker(marker, {type: 'line', class: "gem_checker_warning"});
