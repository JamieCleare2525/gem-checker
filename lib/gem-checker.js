...
export default {
  ...
  config: {
    ...
    maintenance_warning_threshold:{
      title: 'Poorly Maintenance Warning Threshold',
    ...
    enable_highlights:{
      title: 'Enable Warning Highlights',
      description: 'Enable/Disable warning highlights for each line in the Gemfile that meet the Out of Date critera. Functions: "Gemfile Summary"',
      type: 'boolean',
      default: true
    }
  },
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'gem-checker:remove_highlights':() => this.destroy_decorations(),
  ...
  gem_info(){
    ...
    if (editor.getPath().endsWith("Gemfile")){
      ...
      if (!gem_line.includes('path')){
        gem_title = gem_line.trim().split(" ")[1].replace(/["',]/g, "");
  ...
  summarize_gems(gems, gem_lock_lines, file_path){
    ...
    for (i=0; i < gems.length; i++){
      ...
      if (gem_code[0] == "gem" && !gem_code.includes("path:") && !gem_code.includes("git:")){
        ...
        promises.push(detective.cycle_gem_versions(gem_title, i).then(function(version_data){
          ...
          current_version_number = gem_version_list.indexOf(current_version);
          enable_highlights = atom.config.get('gem-checker.enable_highlights');
          summary_writer = new SummaryWriter;
          if(current_version_number == 0){
            // do nothing
          } else if(current_version_number == 1){
            summary_writer.warning_highlight(version_data["row"], "gem_checker_info", enable_highlights);
            ...
            summary_writer.warning_highlight(version_data["row"], "gem_checker_error", enable_highlights);
            ...
            summary_writer.warning_highlight(version_data["row"], "gem_checker_warning", enable_highlights);
      ...
      if(problem_gems == 0){
        ...
        atom.notifications.addWarning("Warning: Summary Completed - Out of Date Gems", {
          detail: problem_gems + " Gems are out of date with the latest versions.\n \n" +
                  ...
                  "Please click the button below to view your summary file.",
          dismissable: true,
          buttons: [
            {
              className: "btn-details",
  ...
  destroy_decorations(){
    ...
    if (editor.getPath().endsWith("Gemfile")){
      ...
      for(i = 0; i < decorations.length; i++){
        properties = decorations[i]["properties"];
        if(properties["class"] == "gem_checker_info" || properties["class"] == "gem_checker_warning" || properties["class"] == "gem_checker_error"){
          marker = decorations[i].getMarker();
          marker.destroy();
        }
      }
    }
