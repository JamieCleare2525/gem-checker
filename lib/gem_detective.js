...
module.exports = class GemDetective{
  ...
  cycle_gem_versions(gem_title, row_number, wait = false){
    self = this;
    if(wait){
    ...
    return new Promise(function(resolve, reject) {
      ...
      $.ajax({url: details_url,
              ...
              success: function(data, status, xhr) {
                if(xhr.status == 200){
                  resolve({versions: data, row: row_number, name: gem_title});
                }else{
              ...
              error: function(xhr, status) {
                if(xhr.status == 429){
                  resolve(self.cycle_gem_versions(gem_title, row_number, true));
                }else{
                  reject({xhr:xhr, name: gem_title});
                }
              }
